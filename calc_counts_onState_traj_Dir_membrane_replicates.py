...
for i in range(1,len(timesOn)):
    if timesOn[i]-timesOn[i-1]==1:
        ...
        events[onEvent,j]=timesOn[i]
print onEvent
#nonzeroOnEvent=(events[1,:]>0).astype(int).nonzero()
#print("events = "+str(nonzeroOnEvent[0][-1]+1))

sizeOnEvent=zeros((onEvent+1,1))
for k in range(onEvent+1):
    j=0
    Lcount[p]=0
    dataCounts=0 
    nonzeroOnEvent=(events[k,:]>0).astype(int).nonzero()
    sizeOnEvent[k]=nonzeroOnEvent[0][-1]+1
    print("Size of on event = " +str(sizeOnEvent[k]))
    while events[k,j]>0:
        #print ("event= %d"%(events[k,j]))
        ...
        Lcount[p] +=1
        j += 1
    counts[p,:]=counts[p,:]/sizeOnEvent[k]
    print("p="+str(p))
    p+=1
...
print("Binned %d data points for lattice size %s to %s"%(dataCounts,counts.shape,outputDir))
